...
short_string::short_string() noexcept : _data(vdupq_n_s8(0)), _size(0) {}

short_string::short_string(std::string_view sv) : short_string() {
  assign(sv);
}

short_string::short_string(const char* str) : short_string() {
  assign(str);
}
...
auto short_string::operator[](size_type pos) const -> const char& {
  return data()[pos];
}

auto short_string::operator[](size_type pos) -> char& {
  return data()[pos];
}

auto short_string::at(size_type pos) const -> const char& {
  if (pos >= _size) {
...
auto short_string::at(size_type pos) -> char& {
  if (pos >= _size) {
    throw std::out_of_range("short_string::at: position out of range");
  }
  return (*this)[pos];
...
auto short_string::data() const noexcept -> const char* {
  return reinterpret_cast<const char*>(&_data);
}

auto short_string::data() noexcept -> char* {
...
auto short_string::begin() const noexcept -> const char* {
  return data();
}

auto short_string::begin() noexcept -> char* {
  return data();
}
...
auto short_string::end() const noexcept -> const char* {
  return data() + _size;
}

auto short_string::end() noexcept -> char* {
  return data() + _size;
}

auto short_string::empty() const noexcept -> bool {
  return (_size == 0);
}

auto short_string::size() const noexcept -> size_type {
  return _size;
}

auto short_string::capacity() const noexcept -> size_type {
  return _capacity;
}

auto short_string::clear() noexcept -> void {
  _size = 0;
  data()[0] = '\0';
}
...
auto short_string::operator==(const short_string& other) const noexcept
    -> bool {
...
auto short_string::operator<(const short_string& other) const noexcept -> bool {
